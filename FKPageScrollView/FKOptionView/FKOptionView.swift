...
public typealias FKOptionViewCellPoint = AutoreleasingUnsafeMutablePointer<FKOptionViewCell>
@objc public protocol FKOptionViewDelegate: NSObjectProtocol{
    ...
    @objc optional func optionView(_ optionView: FKOptionView, didSelectedItemOfIndex index: Int)
}
...
public class FKOptionView: UICollectionView {
    public weak var optionViewDelegate : FKOptionViewDelegate?{
        didSet{
            setOffset()
            setLineView()
        }
    ...
    public var floatIndex: Float = 0.0{
        didSet{
           handeWidthFloaIndex(floatIndex)
        }
    }
    ...
    fileprivate lazy var longLineView: UIView! = {
        ...
        longLine.frame = CGRect(x: 0, y: self.frame.maxY - self.frame.origin.y - 1, width: kDeviceWidth, height: 1)
        return longLine
    }()
    ...
    init(frame: CGRect) {
        ...
        dataSource = self
        bounces = true
        register(FKOptionViewCell.self, forCellWithReuseIdentifier: "FKOptionViewCell")
    }
    convenience init(){
...
extension FKOptionView{
    ...
    fileprivate func handeWidthFloaIndex(_ floatIndex: Float) {
    ...
    private func handleColorOfDecimal(_ decimal: Float) -> UIColor {
        ...
        return FKColor(startColor: startColor!, endColor: endColor!, decimal: CGFloat(decimal))
    }
    ...
    private func cellForOptionViewAtIndex(_ index: Int) ->FKOptionViewCell {
        let cell = cellForItem(at: IndexPath(row: index, section: 0)) as! FKOptionViewCell
        return cell
    }
...
extension FKOptionView: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        var cell: FKOptionViewCell = dequeueReusableCell(withReuseIdentifier: "FKOptionViewCell", for: indexPath) as! FKOptionViewCell
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if let preCell: FKOptionViewCell =  cellForItem(at: IndexPath(row: selectedIndex, section: 0)) as! FKOptionViewCell?  {
            if type == .line {
        ...
        if let currentCell: FKOptionViewCell = cellForItem(at: indexPath) as! FKOptionViewCell? {
            if type == .line {
        ...
        guard optionViewDelegate!.responds(to: #selector(FKOptionViewDelegate.optionView(_:didSelectedItemOfIndex:))) else {
            return
        }
...
public class FKOptionViewCell: UICollectionViewCell{
    var index: Int = 0;
